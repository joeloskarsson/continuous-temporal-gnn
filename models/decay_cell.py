...
class DecayCell(nn.Module):
    def __init__(self, config):
        ...
        if config["decay_type"] == "none":
            # Dummy return values, do not actually matter
            ...
            if config["model"] == "gru": # Works on all nodes combined
        ...
        self.DECAY_CAP = 1000 # For numeric stability

    def compute_inner_states(self, inputs, h_decayed, edge_index):
        raise NotImplementedError()

    def decay_state(self, hidden_state, decay_target, decay_weight, delta_ts):
        # hidden_state: (B, d_h)
        # decay_target: (B, d_h)
        ...
        if self.decay_type == "none":
            return hidden_state # Do not decay state

        decay_factor = torch.exp(-1*torch.clamp(delta_ts*decay_weight, min=0.,
            max=self.DECAY_CAP)) # Shape (B, d_h)

        # hidden-state --> decay_target (decaying as decay factor 1 --> 0)
        return decay_target + (hidden_state - decay_target)*decay_factor

    def forward(self, inputs, hidden_state, decay_target, decay_weight,
            delta_ts, edge_index, edge_weight):
        # inputs: (B, d_in)
        # hidden_state: (B, d_h)
        # delta_ts: (B, 1)
        ...
        h_decayed = self.decay_state(hidden_state, decay_target, decay_weight, delta_ts)
        Wx, Uh = self.compute_inner_states(inputs, h_decayed, edge_index,
                edge_weight)
        ...
        if self.decay_type == "dynamic":
            ...
            # Decay weight
            Wx_decay_weight = W_chunks[-1]
            Uh_decay_weight = U_chunks[-1]
        ...
        return h_decayed, new_h, new_decay_target, decay_weight
